---
layout: people
title: People
permalink: /people/
hero_image: /assets/images/trees.jpg
profiles:
  - name: Daniel Bartl
    photo: Daniel_Bartl.jpeg
    website: sites.google.com/site/danielbartlmath/
    institution: Assistant professor, NUS
    research: High-Dimensional Probability & Statistics, robust Optimization.
  
  - name: Subhro Ghosh
    photo: subhro.jpg
    website: subhro-ghosh.github.io
    institution: Associate professor, NUS
    research: Probability theory, Machine Learning.

  - name: Milind Hegde
    photo: milind-3.jpg
    website: milindhegde.github.io
    institution: Assistant professor, NTU
    research: Probability theory, Statistical Mechanics.
  
  - name: Matteo Mucciconi
    photo: matteo.jpg
    website: matteomucciconi.github.io
    institution: Assistant professor, NUS
    research: Probability theory, Statistical Mechanics.
  
  - name: Nicholas Privault
    photo: Nicholas_Privault.jpeg
    website: personal.ntu.edu.sg/nprivault/index.html
    institution: Professor, NTU
    research: Probability theory, Stochastic Analysis.
  
  - name: Rongfeng Sun
    photo: Rongfeng_Sun.jpeg
    website: sites.google.com/view/rongfengsun/home
    institution: Professor, NUS
    research: Probability theory, Statistical Mechanics.
  
  - name: Hemant Tyagi
    photo: Hemant_Tyagi.jpeg
    website: hemant-tyagi.github.io
    institution: Associate professor, NTU
    research: High-dimensional statistics, Optimization, Statistical learning theory.
  
  - name: Lihan Wang
    photo: Lihan_Wang.jpeg
    website: sites.google.com/view/lihan/about-me
    institution: Assistant professor, NUS
    research: Probability theory, applied analysis.
---

# People
